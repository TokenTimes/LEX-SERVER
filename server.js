...
async function loadPromptFiles() {
  const systemPrompt = await fs.readFile(path.join(__dirname, 'prompts', 'system_prompt.txt'), 'utf8');
  const rules = await fs.readFile(path.join(__dirname, 'rulebook', 'rules.txt'), 'utf8');
  const outputTemplate = await fs.readFile(path.join(__dirname, 'prompts', 'output_template.txt'), 'utf8');
...
app.post('/api/dispute', async (req, res) => {
  ...
  try {
    ...
    const disputeAmount = disputeData.dispute_amount || 
      (() => {
        const amountMatch = disputeData.statement_of_claim.match(/\$?(\d+(?:\.\d{2})?)/);
        return amountMatch ? parseFloat(amountMatch[1]) : 100.00;
      })()
...
Compliance deadline: ${new Date(Date.now() + 5*24*60*60*1000).toISOString()} pursuant to Article 9.1.
...
Return JSON with the complete formatted decision text:`;
  ...
  "decision": {
    ...
    "compliance_deadline": "${new Date(Date.now() + 5*24*60*60*1000).toISOString()}",
    "misconduct_flag": {
    ...
    try {
      ...
      try {
        ...
        if (match) {
          ...
          console.warn(`JSON parse error at position ${errorPos}, attempting to fix...`);
    ...
    const extractReasoningSteps = (decision) => {
      ...
      const claimPreview = disputeData.statement_of_claim.substring(0, 150).replace(/\n/g, ' ').trim();
      const defencePreview = disputeData.statement_of_defence ? 
        disputeData.statement_of_defence.substring(0, 150).replace(/\n/g, ' ').trim() : 
        'No defence submitted';
      
      // Extract facts from finding summary if available
      const factsMatch = findingSummary.match(/ESTABLISHED FACTS[\s\S]*?(?=III\.|$)/i);
      const factsList = factsMatch ? factsMatch[0].match(/•\s*(.+)/g) || [] : [];
      
      // Step 1: Initial Assessment with specific claim details
      steps.push({
        step: 1,
        title: "Initial Case Assessment",
        thought: `Reviewing the ${disputeData.claimant_type}'s claim: "${claimPreview}${disputeData.statement_of_claim.length > 150 ? '...' : ''}"`,
        conclusion: `This is a ${disputeData.dispute_category} dispute where the ${disputeData.claimant_type} seeks $${disputeAmount}. ${
          disputeData.dispute_category === 'defective_item' ? 'The claim involves allegedly defective merchandise.' :
          disputeData.dispute_category === 'non_delivery' ? 'The claim involves items that were allegedly not delivered.' :
          disputeData.dispute_category === 'misrepresentation' ? 'The claim involves alleged misrepresentation of goods/services.' :
      ...
      steps.push({
        ...
        thought: `Analyzing ${evidenceCount > 0 ? `${evidenceCount} piece(s) of submitted evidence` : 'the statements provided'} along with the ${disputeData.claimant_type === 'Buyer' ? 'Seller' : 'Buyer'}'s response: "${defencePreview}${disputeData.statement_of_defence && disputeData.statement_of_defence.length > 150 ? '...' : ''}"`,
        conclusion: `${factsList.length > 0 ? `Key findings: ${factsList.slice(0, 2).join('; ').replace(/•\s*/g, '')}` : 'Evidence has been evaluated for credibility and relevance.'}${
          decision.decision?.misconduct_flag?.misleading_conduct ? ' Note: Indicators of misleading conduct were identified.' : ''
        }`
      });
      ...
      steps.push({
        ...
        thought: `Applying procedural rules specific to ${disputeData.dispute_category.replace(/_/g, ' ')} disputes, including ${
          rulesApplied.includes('Article 5.3') ? 'burden of proof requirements' : ''
        }${rulesApplied.includes('Article 7.3') ? ', incorrect item procedures' : ''}${
          rulesApplied.includes('Article 8.1') ? ', and remedy provisions' : ''
        }.`,
        conclusion: `Applied ${rulesApplied.length} relevant rules: ${rulesApplied.slice(0, 3).join(', ')}${
          rulesApplied.length > 3 ? ` and ${rulesApplied.length - 3} others` : ''
        }. ${disputeData.claimant_type === 'Buyer' ? 'The burden of proof rests with the Buyer to substantiate their claim.' : 'The Seller must demonstrate compliance with transaction terms.'}`
      });
      
      // Step 4: Decision Making with specific outcome
      const confidencePercent = (decision.decision?.confidence_score * 100).toFixed(0);
      
      // Generate confidence reasoning based on various factors
      const getConfidenceReasoning = () => {
        const hasDefence = disputeData.statement_of_defence && disputeData.statement_of_defence.trim().length > 0;
        const hasEvidence = evidenceCount > 0;
        const isMisconductFlagged = decision.decision?.misconduct_flag?.misleading_conduct || decision.decision?.misconduct_flag?.fraudulent_behavior;
      ...
      steps.push({
        ...
        thought: `After weighing the evidence against applicable rules, considering ${
          confidencePercent >= 80 ? 'the strong evidence presented' :
          confidencePercent >= 60 ? 'the preponderance of evidence' :
          'the available evidence with some uncertainties'
        }, a determination has been reached. ${getConfidenceReasoning()}`,
        conclusion: `Ruling in favor of the ${remedyAmount > 0 ? disputeData.claimant_type : (disputeData.claimant_type === 'Buyer' ? 'Seller' : 'Buyer')}. ${
          remedyType === 'full_refund' ? `Full refund of $${remedyAmount} ordered.` :
          remedyType === 'partial_refund' ? `Partial refund of $${remedyAmount} ordered.` :
          remedyType === 'none' ? 'No remedy awarded.' :
...
app.get('/api/admin/reasoning/:disputeId', (req, res) => {
  ...
  if (!reasoningData) {
    return res.status(404).json({ error: 'Reasoning data not found for this dispute' });
  }
...
app.get('/api/test-gemini', async (req, res) => {
  try {
    ...
    const result = await model.generateContent("Say 'Gemini is working!' in JSON format with a field called 'message'");
    const response = await result.response;
